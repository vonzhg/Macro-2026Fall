...
\begin{cautionbox}\footnotesize
Comparisons of this kind are associated with Aruoba, Fern\'andez-Villaverde \& Rubio-Ram\'irez (2006), which sets VFI, perturbation, Chebyshev projection, finite elements and PEA against each other. \textbf{The table above is our own run}, and it covers EGM and time iteration, which that paper does not.
\end{cautionbox}
\end{column}
...
Both are then run on \emph{this} model, side by side. Aiyagari follows in Session 6.
\end{conceptbox}
\vspace{-1mm}
\begin{examplebox}\footnotesize
