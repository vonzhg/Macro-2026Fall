...
\end{columns}
\vspace{1mm}
\src{Adapted from QM \emph{Numerical Methods}, BFGS example. \textbf{Note:} the source's $\alpha_0=0.5$ is a convenient number, not the exact line-search minimizer, which is $5/9$.}
\end{frame}
...
\end{columns}
\vspace{1mm}
\src{Adapted from QM Lec.~4's conjugate-gradient block. \textbf{Note:} the source's printed $d_1=[-80/81,\,40/81]$ has second component $20/81$; see the closing frame.}
\end{frame}
